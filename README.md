...
| **A review that can't skip a file** | The `review` gate takes its file list from **`git` itself** — every changed file is reviewed and the count is stated in the report, so a large changeset never gets the "cut corners" treatment where an agent reviews some files and quietly drops the rest. It then resolves the **callers of every changed public symbol** — via the language server, `grep` as fallback — because that is where a regression actually lands and the diff alone never shows it. The judging stays with an independent review agent working from your own `review_checklist`. |
...
| Hooks | SessionStart · Notification · PreToolUse(commit·merge) | Rule injection + stale-build warning · Teams alerts · commit gate + merge-strategy gate |

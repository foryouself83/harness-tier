...
`git log --grep="git commit"` 도 걸립니다. 그 밖의 프로그램은 그 텍스트를 스크립트로
받을 수 있고 명령만 봐서는 알 수 없는데, 커밋을 놓치는 쪽이 더 나쁜 실수이므로 게이트는
분류를 요구합니다. 그 따옴표가 다른 명령의 것이라면 둘을 `;`·`&&`·줄바꿈으로 나누는 것
으로 충분합니다 — 명령 목록의 각 요소는 따로 읽습니다. 그 명령 자신의 인자라면 나눌 것이
없고, 차단 사유가 지목하는 해법이 그대로 답입니다 — `/flow` 로 작업을 분류하면 tier
마커를 단 명령은 더 이상 미분류 커밋이 아닙니다.

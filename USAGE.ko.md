...
다만 일부러 걸리는 모양이 하나 있습니다 — 인터프리터(`bash`·`sh`·`python3`·`perl`)를
실행하는 **한 명령** 안에 커밋 모양 문구가 따옴표로 함께 들어 있는 경우입니다. 그 텍스트는
인터프리터가 받을 스크립트일 수 있고 명령만 봐서는 어느 쪽인지 알 수 없는데, 커밋을 놓치는
쪽이 더 나쁜 실수이므로 게이트는 분류를 요구합니다. 둘을 `;`·`&&`·줄바꿈으로 나누면 걸리지
않습니다 — 명령 목록의 각 요소는 따로 읽습니다.

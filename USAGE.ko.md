...
커밋 게이트는 명령을 셸이 읽는 방식대로 읽습니다 — `git`, 전역 옵션, 그다음 서브커맨드. 다만
그 호출이 진짜인지는 따지지 않습니다. 그래서 그 형태를 인용만 한 명령(`grep "git commit"`)도
막히며, 정상 동작입니다. 반면 다른 명령이 끝난 뒤에 나오는 언급은 걸리지 않습니다 —
`git log --oneline && echo "now commit"` 은 그대로 실행됩니다.

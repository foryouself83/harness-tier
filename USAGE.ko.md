...
`commit_guide` 파일이 있으면 그 문서를 읽어 스택 사실(스코프 어휘·릴리스 도구·버전
파일)을 우선 적용하고, 메시지 형식은 [`risk-tiers.md`](rules/risk-tiers.md) 가 계속
소유합니다. `/commit` 단독으로도 커밋 하나를 처리하지만 **분류는 하지 않습니다** —
`/flow` 없이 만든 커밋은 여전히 미분류이고 여전히 막힙니다.

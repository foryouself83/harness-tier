...
차단되지 않아야 정상입니다. 게이트는 명령을 셸이 읽는 방식대로 읽고 딱 하나를 묻습니다 —
이 안에 진짜 `git … commit` 이 있는가. 따옴표·주석·heredoc 본문 안의 언급은 텍스트이므로
`grep "git commit"` 도 `git log --oneline && echo "now commit"` 도 그대로 실행됩니다.
다만 일부러 걸리는 모양이 하나 있습니다 — 커밋 모양 문구를 따옴표에 담은 채, 게이트가
읽기 전용으로 아는 프로그램이 아닌 것을 실행하는 명령입니다. 검색·목록 도구는 알고
있습니다(`grep`·`rg`·`ls`·`cat`·`head` 등은 그대로 실행됩니다). `awk`·`sed`·`find`·
`ack`·`ag` 는 자기 인자에 적힌 명령을 실행할 수 있어 제외이고, `git` 자신도 제외라
`git log --grep="git commit"` 도 걸립니다. `less` 도 제외입니다 — 대부분 배포판이 login
profile 에서 설정하는 `LESSOPEN` 이 `less` 가 실행할 명령을 지목하기 때문입니다. `more`
는 여러 호스트에서 같은 프로그램이라 함께 빠집니다. 그래서 `grep 'git commit' f` 는
통과해도 `grep 'git commit' f | less` 는 걸립니다. 걸리는 기준은 따옴표를 지우고 났을 때
`git` 과 서브커맨드가 온전한 두 낱말로 서 있는가입니다 — 그래서 `awk '/git commit/{print}'` 은
낱말 뒤의 `/` 때문에 그대로 실행되고 `sed -e 's|git commit|Y|g'` 는 걸립니다. 같은 명령
안에 `$( … )`·백틱·`$(( … ))` 가 하나라도 있으면 읽기 전용 면제도 사라집니다 — 치환이
출력하는 것은 명령이기 때문입니다. 별도의 명령으로 나누면 검색 도구는 다시 알아봅니다.
읽기 전용 명령 앞에 붙은 환경변수 대입(`LC_ALL=C grep …`)도 면제를 없앱니다 — 대입은
프로그램 옆에 서는 것이 아니라 안으로 들어가고, `LESSOPEN` 이 `less` 로 하여금 아무도
명령에 적지 않은 명령을 실행하게 만드는 것이 그 예입니다. 그 밖의 프로그램은 그
텍스트를 스크립트로 받을 수 있고 명령만 봐서는 알 수 없는데, 커밋을 놓치는 쪽이 더 나쁜
실수이므로 게이트는 분류를 요구합니다. 그 따옴표가 다른 명령의 것이라면 둘을 `;`·`&&`·
줄바꿈으로 나누는 것으로 충분합니다 — 명령 목록의 각 요소는 따로 읽습니다. 그 명령
자신의 인자라면 나눌 것이 없고, 차단 사유는 미분류 커밋이 받는 것과 같습니다 — `/flow`
로 작업을 분류하고 그 tier 의 게이트 증거를 갖추면 실행됩니다. 커밋 자신이 넘어야 하는
기준과 같습니다.
그게 아닌데도 막힌다면 차단 사유를 읽으세요: python3 이 없으면 게이트는 호출과 언급을
구별할 수 없어 추측 대신 막고, 그 메시지가 설치 방법을 지목합니다. 그게 아니면 호스트에 복사된 게이트
스크립트가 플러그인보다 낡은 것입니다 — `/flow-init` 을 다시 실행하세요.
...
갱신이 기다리고 있다는 것은 세션이 알려줍니다: 시작 시점에 훅이 로드된 빌드와 마켓플레이스가
게시하는 버전을 비교해, 마켓플레이스가 더 앞서 있을 때 한 줄로 알립니다 — 릴리스 후보는
게시된 것보다 앞서 있고, 그 유지보수자에게 갱신하라고 하면 더 낡은 핀을 가져오는 처방을
지목하게 됩니다. 두 숫자 모두 로컬 파일 — Claude Code 가 설치
캐시 옆에 두는 마켓플레이스 클론 — 이라 네트워크를 타지 않고, 갱신되지 않은 클론은 아무 말도
하지 않습니다.

`/plugin` 으로 갱신한 뒤 `/flow-init` 을 다시 실행하세요. 갱신만으로는 부족합니다: 호스트의
스크립트 사본은 함께 바뀌지 않습니다(복사본이라서). `/flow-init` 을 다시 실행하면 스크립트·정책
파일을 다시 복사하고 게이트 경로를 보정합니다(재동기화는 비대화로 항상 먼저 실행). 빠진 config
슬롯이 있으면 보충을 제안하고, 그 외에는 무엇을 재설정할지 물어봅니다(아무것도 안 고르면
재동기화만; 설정값·웹훅은 보존).

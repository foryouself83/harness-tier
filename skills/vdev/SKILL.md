---
name: vdev
description: MANDATORY first step for ALL development work — invoke BEFORE starting any code change, feature, fix, or free-text dev request (also accepts an ALM task-id like SDAL-0091). Classifies the task as Docs/Dev by risk, confirms the tier, runs the matching workflow, and records the gate evidence the commit hook enforces; skipping it leaves the commit unclassified and the commit gate blocks it. Staging/Release apply at integration→staging / staging→production.
allowed-tools: Bash, Read, Grep, Glob, Skill, AskUserQuestion, Edit, Write
argument-hint: "[free-text request | ALM task-id]"
---

# Vdev — Risk-Tiered Workflow Router

Classify the incoming work by risk, confirm the tier, run the matching workflow,
and record gate evidence under `.claude/vway-kit/.vdev/` so the `git commit` hook
enforces the tier's required gates.

**Source of truth**: [`risk-tiers.md`](../../rules/risk-tiers.md) (criteria, skill
gate, per-tier steps) and [`vdev-tiers.yaml`](../../vdev-tiers.yaml) (tier→gates the
commit hook enforces). Read them first; do not duplicate their content.

Branch names referenced below come from `vdev-config.branches`
(`integration` / `staging` / `production`). Domain-review items come from
`vdev-config.review_checklist`, per-module pre-checks from `vdev-config.modules`,
and the ALM project/workitem from `vdev-config.teamer`.
...
- **$ARGUMENTS** — a free-text request or an ALM task-id (e.g. `DEV-0952`).
- If empty, ask the user what the task is.

## Phase 0 — Resolve input

- If `$ARGUMENTS` matches `^[A-Z]+-\d+$` (an ALM task-id), invoke the
  [`task-import` skill](../task-import/SKILL.md) (via the Skill tool) with `$ARGUMENTS` first — it imports the ALM
  context and **scaffolds** the task doc (Content filled; analysis/plan left as
  placeholders). Use the scaffolded doc as the request. **Remember the task-id**
  — it triggers a final [`task-sync` skill](../task-sync/SKILL.md).
...
Output the verdict — tier, reason, gates (from [`vdev-tiers.yaml`](../../vdev-tiers.yaml)):
...
integration/staging/production branch (`vdev-config.branches`). See
...
```bash
# 증거 디렉터리가 git 에 절대 노출되지 않게 보장(멱등). /vdev-init 선행 없이도 안전하고,
# tier 마커를 쓰기 *전에* 무시 규칙을 둬 untracked 노출 창을 닫는다. (이미 추적 중인
# 증거는 /vdev-init 의 untrack 단계가 git rm --cached 로 정리한다.)
grep -qxF '.claude/vway-kit/.vdev/' .gitignore 2>/dev/null || printf '\n.claude/vway-kit/.vdev/\n' >> .gitignore
mkdir -p .claude/vway-kit/.vdev
echo "<tier>:$(git branch --show-current)" > .claude/vway-kit/.vdev/tier   # docs | dev
```

## Phase 3 — Dispatch

Record each completed gate as `.claude/vway-kit/.vdev/<gate>.done`. `precommit`
(모듈 lint/static/import_lint/test) and `security-scan` are executed by the
commit hook itself — no marker (both are ordinary `gates` entries; removing
one from a tier's list in [`vdev-tiers.yaml`](../../vdev-tiers.yaml) disables
it for that tier).
...
> (`superpowers@claude-plugins-official`, e.g. via `/plugin`) and re-run `/vdev`.
...
2. Invoke the `doc-sync` skill to harmonize the doc set (index `CLAUDE.md` + per-service docs +
   rule dirs from `vdev-config.doc_sync`; also reconciles code↔doc drift). On pass
...
1. **Enter `superpowers:using-superpowers`** — it drives the pipeline automatically
   ...
   Feed the resolved request from Phase 0 in as the task. If Phase 0 scaffolded a
   task doc via [`task-import` skill](../task-import/SKILL.md), fill its placeholder sections
   (Codebase Analysis / Implementation Sequence / Implementation Plan) from the
   imported ALM content as `superpowers` analyzes and plans.
2. Apply the project overlays `superpowers` does not know about:
   ...
   - **Domain review** — an independent `general-purpose` review agent (separate
     context) against `vdev-config.review_checklist` (regression, cross-service
     contract, DB/migration & transactions, async task idempotency, API errors).
     On pass → `touch .claude/vway-kit/.vdev/review.done`.
   - **invoke the `doc-sync` skill** (not part of `superpowers`) → `touch .claude/vway-kit/.vdev/doc-sync.done`.
...
- **Staging** (integration → staging): regression `review` (independent
  ...
  `touch .claude/vway-kit/.vdev/{review}.done`, then commit on the
  staging branch.
- **Release** (staging → production): Staging gates **plus** `/code-review` at
  `ultra` effort (extra independent layer) and `/security-review` →
  `touch .claude/vway-kit/.vdev/security.done`, then commit on the production branch.
  Deploy (project-specific / offline) — not gated; the production-branch commit
  is the gate.

The commit hook ([`vdev_gate_check.py`](../../scripts/vdev_gate_check.py)) blocks
the staging/production commit until those markers exist.

## Phase 4 — Finalize

After the commit/merge completes:

1. **If the entry was an ALM task-id**, invoke the [`task-sync` skill](../task-sync/SKILL.md) (via the Skill tool) with `<task-id>` to sync the result summary back to the ALM.
2. Clear the vdev state:
   ```bash
...
2. **Record gate evidence honestly** — `touch .claude/vway-kit/.vdev/<gate>.done` only
...
6. **[`risk-tiers.md`](../../rules/risk-tiers.md) and
   [`vdev-tiers.yaml`](../../vdev-tiers.yaml) are the source of truth** — if this

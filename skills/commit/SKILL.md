...
supplies the flag itself is not one, so the command reads as something that is not a commit
and every gate behind it is skipped without a word. A variable used as a

...
| **파일을 빠뜨릴 수 없는 리뷰** | `review` 게이트는 변경 파일 목록을 **`git` 에서 직접** 가져옵니다 — 변경된 모든 파일을 리뷰하고 그 개수를 보고서에 명시하므로, 큰 변경분에서 에이전트가 일부만 보고 나머지를 조용히 넘기는 일이 없습니다. 여기에 **변경된 public 심볼의 호출자**까지 language server(없으면 `grep`)로 확인합니다 — 회귀가 실제로 터지는 자리이고 diff 만으로는 드러나지 않기 때문입니다. 판정은 독립 리뷰 에이전트가 팀의 `review_checklist` 를 기준으로 수행합니다. |
...
| 훅 | SessionStart · Notification · PreToolUse(commit·merge) | 규칙 주입 + 구버전 로드 경고 · Teams 알림 · 커밋 게이트 + 머지 전략 게이트 |

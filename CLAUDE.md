...
tests/           pytest over scripts/ · test_skills.py (skill FILES: frontmatter/links/refs + the git
                 commands skills and rules/ issue vs the gate's self-filter) · test_evals.py (model-free half of evals/)
...
6. **Worktree re-designation stays FAIL-OPEN** — `CLAUDE_PROJECT_DIR` is fixed at session start, so `precommit-runner.sh` detects the commit's actual worktree by branch-key (`flow_gate_check.py --resolve-worktree` → `_harness_paths.working_root`) and re-points `ROOT` to it, so status/diff/tier-marker/module-lint all read the worktree. Any uncertainty — detached HEAD · `--git-common-dir` mismatch · no worktree · parse/exception · invocations naming different directories — must return main (= current behavior); never newly block. Returning main is only silent when main carries the same classification, so keep the uncertain set small: two `git -C <same wt> commit` in one command (commit-then-amend) name one directory and must resolve to it. Same-repo identity uses **`--git-common-dir` equality, never a path prefix** (sibling `…/kit` vs `…/kit-feature` must not false-match). The commit self-filter must keep matching `git -C <wt> commit` — a path quoted with EITHER quote character, spaces included — not just the bare `git commit` substring, else a worktree commit slips the gate entirely; and no option token may cross whitespace by pairing one string's closing quote with a later string's opening one, which made `git log … && echo "… commit"` a commit the gate denied. A regex cannot mask literals, so a fully quoted `git commit` still reads as one. `merge` shares the grammar (`test_skills.py`). **The runner's filter and the Python grammar must agree on what an invocation is** — the first decides whether the gate runs at all, the second what it runs against, so a spelling only one accepts is the gate off in silence rather than a narrower gate: `/usr/bin/git … commit` reached the runner but resolved no worktree, leaving `ROOT` on a clean main that exited 0. A directory prefix therefore names the same program (it must end in a separator, so `mygit` stays rejected), and a backslash makes the next character literal so `\'` opens no span — without that the option token ends at the quote and the runner exits before every gate, on the `'\''` spelling the commit skill's own template emits. `test_skills.py` pins the two halves to a corpus of real invocations, since comparing the two shell patterns to each other lets both drift together. A skill's `git commit`/`git merge` must spell its flags **literally** for the same reason — the hook reads the command unexpanded, and a variable standing where a flag belongs matches neither filter (`test_skills.py`).

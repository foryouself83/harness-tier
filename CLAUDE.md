...
- **Three verification layers**, independent (per-gate mechanism → [`rules/risk-tiers.md`](rules/risk-tiers.md) · [`flow-tiers.yaml`](flow-tiers.yaml)):
  ...
  The `wiki` runtime gate runs in its **own** `--wiki-check` step (not the module-command channel —
  that channel reads any nonzero exit as a failure, which would make an internal error block every commit),
  whenever `flow-config.wiki` is enabled — read-only, on every tier including docs. The graph itself is
